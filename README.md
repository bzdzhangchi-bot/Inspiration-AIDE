...
- Added a built-in Starter Project that opens on first launch, including a Chinese getting-started guide, HTML/Markdown preview demos, and a Spring Boot sample workspace.
- Refined the Project workspace with manual refresh, clearer project re-open handling, and better tab overflow behavior plus JetBrains-style tab closing actions.
- Reworked Claude CLI mode into an explicit Terminal handoff flow so chat no longer mirrors volatile CLI output back into the message list.
...
- 新增内置 Starter Project，首次启动即可直接进入，并附带中文新手引导、HTML/Markdown 预览示例和 Spring Boot 示例工程。
- 强化 Project 工作区体验，加入手动刷新、重复打开项目时的定位反馈，以及更接近 JetBrains 的标签页管理能力。
- 重构 Claude CLI 模式为明确的 Terminal 交接流程，不再把不稳定的 CLI 输出镜像回聊天消息列表。
