...
Assistant Desk is a macOS desktop AI coding workspace that brings together:
...
- Fixed an intermittent issue where Markdown `Edit` and `Preview` switching could stop responding after moving between previewable files.
- Kept Markdown TOC anchors aligned with rendered preview anchors for more reliable in-document navigation.
- Includes the packaged-app performance improvements introduced in `0.1.2`.
...
2. Open the `.dmg` and drag **Assistant Desk** into **Applications**.
3. Launch the app.
...
```bash
release/Assistant Desk-<version>-arm64.dmg
release/Assistant Desk-<version>-arm64.zip
release/Assistant Desk-<version>-x64.dmg
release/Assistant Desk-<version>-x64.zip
```
...
Assistant Desk 是一款 **macOS 桌面端 AI 编程工作台**，把以下能力放到一个应用里：
...
- 保持 Markdown 目录锚点与预览区实际锚点一致，减少文档内跳转错位。
- 同时包含 `0.1.2` 中已经完成的打包版性能优化。
...
2. 打开 `.dmg`，将 **Assistant Desk** 拖入 **应用程序**。
3. 启动应用。
