...
- Added integrated Git workspace support inside the desktop workspace.
- Fixed a workspace-context render loop that could cause unnecessary rerenders.
- Expanded native agent workflow coverage for local coding tasks.
...
- 新增桌面工作区内置 Git workspace 支持。
- 修复了 workspace context 可能引发不必要重复渲染的问题。
- 扩展了本地编码任务下的原生 agent workflow 覆盖范围。
