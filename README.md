...
- Fixed Markdown inline code rendering so single backtick fragments such as `.jsonl` stay inline instead of expanding into block code.
- Restored Mermaid fenced-code rendering in the workspace preview after the Markdown renderer adjustment.
- Simplified inline code styling to a quieter neutral treatment that reads cleanly in both chat and preview surfaces.
...
- 修复 Markdown 行内代码渲染，像 `.jsonl` 这类单反引号包裹的片段不再被错误撑成块级代码。
- 修复工作区预览中的 Mermaid fenced code 渲染，避免在 Markdown 渲染调整后失效。
- 将行内代码样式收敛为更克制的中性视觉，在聊天区和预览区都更简洁。
